...
def sync_to_mobile():
    ...
    try:
        ...
        # ── Push approved labour_logs to Supabase so mobile sees hours ────
        ll_rows = fetch_df("SELECT * FROM labour_logs WHERE work_date >= date('now','-30 days')")
        for _, r in ll_rows.iterrows():
            supa_push("labour_logs", {
...
def sync_from_mobile():
    ...
    try:
        ...
        # ── Pull labour_logs from mobile ──────────────────────────────────
        ll_data = supa_pull("labour_logs")
        for ll in ll_data:
            emp  = ll.get("employee","")
            ...
            existing = fetch_df("""SELECT id FROM labour_logs
                WHERE employee=? AND work_date=? AND job_id=? AND hours=?""",
                (emp, wdate, jid, hrs))
            if existing.empty:
                execute("""INSERT INTO labour_logs
                    (work_date, job_id, employee, hours, hourly_rate, note)
                    VALUES (?,?,?,?,?,?)""",
                    (wdate, jid, emp,
                     hrs, ll.get("hourly_rate", 0), ll.get("note","")))
        # ── Pull job_photos from mobile ───────────────────────────────────
        photos_data = supa_pull("job_photos")
        for ph in photos_data:
            jid    = ph.get("job_id","")
            pdate  = ph.get("photo_date","")
            uploader = ph.get("uploaded_by","")
            caption  = ph.get("caption","")
            if not jid or not pdate: continue
            existing = fetch_df("""SELECT id FROM job_photos
                WHERE job_id=? AND photo_date=? AND uploaded_by=? AND caption=?""",
                (jid, pdate, uploader, caption))
            if existing.empty:
                import base64 as _b64
                photo_bytes = None
                if ph.get("photo_data"):
                    try: photo_bytes = _b64.b64decode(ph["photo_data"])
                    except: photo_bytes = None
                execute("""INSERT INTO job_photos
                    (job_id, photo_date, caption, photo_data, uploaded_by)
                    VALUES (?,?,?,?,?)""",
                    (jid, pdate, caption, photo_bytes, uploader))
    except Exception as _se:
        pass
...
if USE_POSTGRES:
    try:
        ...
        for _sql in [
            ...
            "CREATE TABLE IF NOT EXISTS job_photos (id SERIAL PRIMARY KEY, job_id TEXT, photo_date TEXT, caption TEXT DEFAULT \'\', photo_data TEXT, uploaded_by TEXT DEFAULT \'\', synced INTEGER DEFAULT 0)",
...
if not st.session_state["authenticated_user"]:
    ...
    <div style="text-align:center;margin-bottom:2rem">
        <div style="font-size:38px;font-weight:700;letter-spacing:.1em;color:#2dd4bf;font-family:'Barlow Semi Condensed',sans-serif">LIMITLESS</div><div style="font-size:13px;font-weight:600;letter-spacing:.25em;color:#64748b;font-family:'Barlow Semi Condensed',sans-serif">JOB MANAGEMENT</div>
    </div>
    """, unsafe_allow_html=True)
